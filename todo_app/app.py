...
from todo_app.data.trello_items import get_trello_board_id, get_trello_cards, get_trello_lists, create_new_trello_card, move_trello_card, ViewModel, get_trello_cards_from_list, get_trello_list_id, archive_trello_card
        ...
        if request.form.get("donecheck_" + card.id) == card.id:
            for l in lists:
                if l['name'] == "Done":
                    desired_list_id = l['id']
                    move_trello_card(card.id, desired_list_id)
        if request.form.get("archivecheck_" + card.id) == card.id:
